...
import {
  ...
  XCircle, PauseCircle, Pencil, Trash2, Lock,
} from "lucide-react";
...
const CYCLE_LABEL = { monthly: "Mensal", yearly: "Anual", lifetime: "Vitalicio" };
...
function PlanBadge({ plan, planName, planColor }) {
  const name = plan?.name || planName || "--";
  const color = plan?.color || planColor || "#818cf8";
  return (
    <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-xs font-medium" style={{ background: `${color}33`, color }}>
      {plan?.is_public === false && <Lock className="w-3 h-3" />}
      {name}
    </span>
  );
}
...
function RenewModal({ sub, onClose, onDone }) {
  ...
  const [method, setMethod] = useState(sub?.payment_method || "pix");
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      setError(err?.response?.data?.detail || "Erro ao renovar.");
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
      <div className="bg-[#161B22] border border-white/5 rounded-2xl w-full max-w-md shadow-2xl">
        ...
        <form onSubmit={submit} className="p-5 space-y-4">
          ...
          <p className="text-sm text-gray-400">Restaurante: <span className="text-gray-100 font-medium">{sub?.restaurant?.name || sub?.restaurant_name || "--"}</span></p>
          <div>
          ...
          <div>
            <label className="text-xs text-gray-400 mb-1 block">Metodo de pagamento</label>
...
function ActivationModal({ subscription, plans, restaurants, onClose, onDone }) {
  const initialRestaurantName = subscription?.restaurant?.name || subscription?.restaurant_name || "";
  const [form, setForm] = useState(() => ({
    restaurant_id: subscription?.restaurant_id || "",
    plan_id: subscription?.plan_id || "",
    billing_cycle: subscription?.billing_cycle || subscription?.cycle || "monthly",
    ...
    expires_at: subscription?.expires_at ? String(subscription.expires_at).slice(0, 10) : "",
    notes: subscription?.notes || "",
  }));
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState("");
  const [restSearch, setRestSearch] = useState(initialRestaurantName);

  const set = (key, value) => setForm((prev) => ({ ...prev, [key]: value }));
  const selectedPlan = plans.find((plan) => String(plan.id) === String(form.plan_id));
  const filteredRests = restaurants
    .filter((restaurant) => restaurant.name.toLowerCase().includes(restSearch.toLowerCase()))
    .slice(0, 8);

  const priceForCycle = (plan, cycle) => {
    if (!plan) return "";
    if (cycle === "lifetime") return plan.price_lifetime ?? plan.price_yearly ?? plan.price_monthly ?? 0;
    if (cycle === "yearly") return plan.price_yearly ?? 0;
  ...
  const handlePlanChange = (planId) => {
    ...
    set("plan_id", planId);
    if (plan) set("amount", priceForCycle(plan, form.billing_cycle));
  };

  const handleCycleChange = (cycle) => {
    set("billing_cycle", cycle);
    if (selectedPlan) set("amount", priceForCycle(selectedPlan, cycle));
    if (cycle === "lifetime") set("expires_at", "");
  };

  const submit = async (event) => {
    event.preventDefault();
    if (!form.restaurant_id || !form.plan_id) {
      setError("Selecione restaurante e plano.");
      return;
    }
    setSaving(true);
    setError("");
    try {
      const payload = {
        ...
        expires_at: form.billing_cycle === "lifetime" ? null : (form.expires_at || null),
      };
      if (subscription?.id) await api.put(`/super/subscriptions/${subscription.id}`, payload);
      else await api.post("/super/subscriptions", payload);
      toast.success(subscription?.id ? "Ativacao atualizada" : "Ativacao criada");
      onDone();
    } catch (err) {
      setError(err?.response?.data?.detail || err?.response?.data?.message || "Erro ao salvar ativacao.");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
      <div className="bg-[#161B22] border border-white/5 rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-2xl">
        <div className="flex items-center justify-between p-5 border-b border-white/5">
          <h2 className="font-bold text-gray-100">{subscription?.id ? "Editar Ativacao" : "Nova Ativacao"}</h2>
          <button onClick={onClose} className="text-gray-400 hover:text-gray-200"><XCircle className="w-5 h-5" /></button>
        </div>
        <form onSubmit={submit} className="p-5 space-y-4">
          {error && <p className="text-sm text-red-400 bg-red-500/10 border border-red-500/20 rounded-lg px-3 py-2">{error}</p>}
            ...
            {restSearch && !form.restaurant_id && (
              <div className="bg-[#1E2430] border border-white/10 rounded-lg overflow-hidden">
                {filteredRests.map((restaurant) => (
                  <button key={restaurant.id} type="button" onClick={() => { set("restaurant_id", restaurant.id); setRestSearch(restaurant.name); }}
                    className="w-full text-left px-3 py-2 text-sm text-gray-200 hover:bg-white/5">{restaurant.name}</button>
                ))}
          ...
          <div>
            ...
            <select value={form.plan_id} onChange={(e) => handlePlanChange(e.target.value)} className="w-full bg-[#1E2430] border border-white/10 text-gray-100 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-indigo-500">
              ...
              {plans.map((plan) => (
                <option key={plan.id} value={plan.id}>{plan.name}{plan.is_public === false ? " (oculto)" : ""}</option>
              ))}
            </select>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select value={form.billing_cycle} onChange={(e) => handleCycleChange(e.target.value)} className="w-full bg-[#1E2430] border border-white/10 text-gray-100 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-indigo-500">
          ...
          {subscription?.id && (
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <label className="text-xs text-gray-400 mb-1 block">Vence em</label>
                <input type="date" disabled={form.billing_cycle === "lifetime"} value={form.expires_at || ""} onChange={(e) => set("expires_at", e.target.value)} className="w-full bg-[#1E2430] border border-white/10 text-gray-100 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-indigo-500 disabled:opacity-50" />
              </div>
            </div>
          )}
          <div>
            <label className="text-xs text-gray-400 mb-1 block">Codigo afiliado</label>
...
export default function Activations() {
  ...
  const toggleStatus = async (sub) => {
    ...
    setToggling(sub.id);
    try {
      await api.put(`/super/subscriptions/${sub.id}/status`, { status: newStatus });
      toast.success(newStatus === "active" ? "Assinatura ativada" : "Assinatura suspensa");
      await load();
    } catch (err) {
  ...
  const deleteSub = async (sub) => {
    const name = sub.restaurant?.name || sub.restaurant_name || "este restaurante";
    if (!window.confirm(`Apagar a ativacao de ${name}?`)) return;
    setDeleting(sub.id);
    try {
      await api.delete(`/super/subscriptions/${sub.id}`);
      toast.success("Ativacao apagada");
      await load();
    } catch (err) {
      toast.error(err?.response?.data?.detail || "Erro ao apagar ativacao");
    } finally {
      setDeleting(null);
    }
  };

  const kpiActive = subs.filter((sub) => sub.status === "active").length;
  const kpiTrial = subs.filter((sub) => sub.status === "trial").length;
  const kpiExpiring = alerts.expiring?.length || 0;
  const kpiOverdue = subs.filter((sub) => sub.status === "overdue").length;
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-100">Ativacoes</h1>
          <p className="text-sm text-gray-400 mt-0.5">Gerencie as assinaturas dos restaurantes</p>
        </div>
        <button onClick={() => setActivationModal({})} className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-xl text-sm font-medium">
          <Plus className="w-4 h-4" /> Nova Ativacao
        </button>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          <div key={kpi.label} className="bg-[#161B22] border border-white/5 rounded-2xl p-5">
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <select value={planFilter} onChange={(e) => setPlanFilter(e.target.value)} className="bg-[#1E2430] border border-white/10 text-gray-100 rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-indigo-500">
          ...
          {plans.map((plan) => <option key={plan.id} value={plan.id}>{plan.name}{plan.is_public === false ? " (oculto)" : ""}</option>)}
        </select>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-white/5">
                  {["Restaurante", "Plano", "Status", "Valor", "Ciclo", "Vence em", "Afiliado", "Acoes"].map((header) => (
                ...
                {subs.map((sub) => {
                  ...
                  return (
                    <tr key={sub.id} className="hover:bg-white/[0.02]">
                      <td className="px-4 py-3 text-gray-100 font-medium">{sub.restaurant?.name || sub.restaurant_name || "--"}</td>
                      <td className="px-4 py-3"><PlanBadge plan={sub.plan} planName={sub.plan_name} planColor={sub.plan_color} /></td>
                      <td className="px-4 py-3"><StatusBadge status={sub.status} /></td>
                      <td className="px-4 py-3 text-gray-300">{brl(sub.amount)}</td>
                      <td className="px-4 py-3 text-gray-400">{CYCLE_LABEL[sub.billing_cycle || sub.cycle] || sub.billing_cycle || sub.cycle}</td>
                      <td className="px-4 py-3">
                        ...
                        ) : <span className="text-gray-500">Vitalicio</span>}
                      </td>
                      <td className="px-4 py-3 text-gray-400 text-xs">{sub.affiliate_code || "--"}</td>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          <button onClick={() => setActivationModal(sub)} className="text-xs border border-white/10 hover:border-indigo-500/50 text-gray-400 hover:text-indigo-400 rounded-lg px-2 py-1 flex items-center gap-1">
                            <Pencil className="w-3 h-3" /> Editar
                          </button>
                          <button onClick={() => setRenewModal(sub)} disabled={(sub.billing_cycle || sub.cycle) === "lifetime"} className="text-xs border border-white/10 hover:border-indigo-500/50 text-gray-400 hover:text-indigo-400 rounded-lg px-2 py-1 flex items-center gap-1 disabled:opacity-40 disabled:hover:text-gray-400">
                          ...
                          <button onClick={() => deleteSub(sub)} disabled={deleting === sub.id} className="text-xs border border-white/10 hover:border-red-500/50 text-gray-400 hover:text-red-400 rounded-lg px-2 py-1 flex items-center gap-1 disabled:opacity-50">
      ...
      {activationModal !== null && (
        <ActivationModal
          subscription={activationModal?.id ? activationModal : null}
          plans={plans}
          restaurants={restaurants}
          onClose={() => setActivationModal(null)}
          onDone={() => { setActivationModal(null); load(); }}
        />
      )}
    </div>
  );
